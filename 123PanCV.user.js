...
// @description:zh-cn 123云盘提取码管理脚本：自动保存、表格管理、跳转链接、复制提取码、导入导出、防止自动写入剪贴板
...
(function () {
    ...
    navigator.clipboard.writeText = async function (text) {
        console.log("已阻止123云盘写入剪贴板内容:", text);
    ...
    function findFileNames() {
        ...
        // 方法2: 查找包含文件名的父元素，然后组合所有 file-name-line 的内容
        document
            .querySelectorAll('.file-name-display, [class*="file-name"]')
            .forEach(el => {
                // 获取所有 file-name-line 子元素的文本并组合
                const nameParts = Array.from(
                    el.querySelectorAll(".file-name-line")
                )
    ...
    <div style="font-size: 14px; opacity: 0.9; margin-top: 2px;">共 ${
        data.length
    } 条记录</div>
    </div>
    </div>
    ...
    `;
        ...
        overlay.addEventListener("mouseover", e => {
            ...
            // GitHub图标悬停效果
            if (
                target.closest('a[href="https://github.com/Chumor/123PanCV"]')
            ) {
                target.style.background = "rgba(255,255,255,0.3)";
            }
        });
