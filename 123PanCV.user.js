...
// @version      1.2.250920.1
// @description  A lightweight Tampermonkey userscript for 123Pan with clipboard write block
// @description:zh-CN  自动保存提取码、表格化管理，并阻止网页写入剪贴板
...
(function () {
  ...
  function findCodes() {
    ...
    document.cookie.split(';').forEach(c => {
      ...
      if (kv.length === 2 && /^[A-Za-z0-9]{4}$/.test(kv[1])) out.add(kv[1]);
    });
